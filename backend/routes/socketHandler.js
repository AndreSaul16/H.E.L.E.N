    ...
    io.on('connection', (socket) => {
        ...
        socket.on('audio-data', async (audioBlob) => {
            ...
            try {
                 ...
                 */
                const response = await services.chatgpt.sendMessage(transcript);
                ...
                let chunkBuffer = []; // Buffer para acumular chunks pequeños
                const MIN_CHUNK_SIZE = 1000; // Tamaño mínimo en bytes
                ...
                audioStream.on('data', (chunk) => {
                    ...
                    // Acumular chunks en el buffer
                    chunkBuffer.push(chunk);
                    const bufferSize = chunkBuffer.reduce((sum, c) => sum + c.length, 0);

                    // Si el buffer alcanzó el tamaño mínimo O es un chunk grande, enviar
                    if (bufferSize >= MIN_CHUNK_SIZE || chunk.length >= MIN_CHUNK_SIZE) {
                        // Combinar todos los chunks del buffer
                        const combinedChunk = Buffer.concat(chunkBuffer);
                        chunkBuffer = []; // Limpiar buffer

                        chunkCount++;

                        // Convertir a Base64
                        const base64Chunk = combinedChunk.toString('base64');
                        ...
                        console.log(`📦 [WebSocket] Chunk ${chunkCount} enviado (${combinedChunk.length} bytes)`);
                    } else {
                        console.log(`🔄 [WebSocket] Buffer acumulando: ${bufferSize} bytes (min: ${MIN_CHUNK_SIZE})`);
                    }
                });

                audioStream.on('end', () => {
                    // Enviar cualquier chunk que quede en el buffer
                    if (chunkBuffer.length > 0) {
                        ...
                        console.log(`📦 [WebSocket] Último chunk enviado (${combinedChunk.length} bytes)`);
                        chunkBuffer = [];
                    }
