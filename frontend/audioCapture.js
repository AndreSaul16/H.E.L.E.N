...
class AudioCaptureVAD {
    ...
    async start() {
        try {
            ...
            // ============================================================
            /**
             * CONCEPTO: ¿Qué hace MicVAD.new()?
             * ---------------------------------
             * - Descarga el modelo de IA (Silero VAD) si no está en caché
             * - Pide permiso al micrófono
             * - Configura el análisis de audio en tiempo real
             * 
             * ANALOGÍA: Contratar un guardia de seguridad
             * Le das instrucciones (configuración) y empieza a vigilar
             * tu casa (micrófono) automáticamente
             */
    ...
    async stop() {
        ...
        if (this.vad) {
            ...
            this.vad.destroy(); // Liberar recursos
            this.vad = null;
        }
